/** Hafs — standard verse counts per surah (1…114). Used when mapping audio time → ayah for a surah not currently loaded in the reader. */
const MEDINA_VERSE_COUNTS: readonly number[] = [
...
export function getVerseCountForSurah(surahNumber: number): number {
  if (!Number.isFinite(surahNumber) || surahNumber < 1 || surahNumber > 114) {
    return 1;
  }
  return MEDINA_VERSE_COUNTS[surahNumber - 1] ?? 1;
}
